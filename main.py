...
async def vector_transport_resolver(url: str) -> str:
    """
    Optimized version for faster playback with caching
    """
...
# Track current playback position for seek functionality
current_playback_positions = {}
...
async def help_music_callback(_, callback_query):
    text = (
        ...
        ">➜ `/seek <seconds>`\n"
        "   • Forward the current song by specified seconds. (Admins only)\n\n"
        ">➜ `/seekback <seconds>`\n"
        "   • Rewind the current song by specified seconds. (Admins only)"
    )
...
async def update_progress_caption(
    ...
    while True:
        ...
        # Update current playback position for seek functionality
        current_playback_positions[chat_id] = elapsed
...
async def seek_forward_handler(client, message):
    ...
    if chat_id not in current_playback_positions:
    ...
    try:
        ...
        current_pos = current_playback_positions.get(chat_id, 0)
        new_position = current_pos + seek_seconds

        # Get current song info
        if chat_id in chat_containers and chat_containers[chat_id]:
            current_song = chat_containers[chat_id][0]
            total_duration = current_song.get('duration_seconds', 0)
            
            if new_position >= total_duration:
                await message.reply("❌ Cannot seek beyond the song duration.")
                return

            # Restart playback from new position
            await message.reply(f"⏩ Seeking forward by {seek_seconds} seconds...")
            ...
            current_song['seek_position'] = new_position
            await fallback_local_playback(chat_id, message, current_song)
...
async def seek_backward_handler(client, message):
    ...
    try:
        ...
        current_pos = current_playback_positions.get(chat_id, 0)
        new_position = max(0, current_pos - seek_seconds)
        ...
        if chat_id in chat_containers and chat_containers[chat_id]:
            ...
            # Restart playback from new position
            await message.reply(f"⏪ Seeking backward by {seek_seconds} seconds...")
        ...
        await message.reply(f"❌ Error during seekback: {str(e)}")
...
async def stream_end_handler(_: PyTgCalls, update: StreamEnded):
    ...
    if chat_id in chat_containers and chat_containers[chat_id]:
        ...
        if chat_id in current_playback_positions:
            del current_playback_positions[chat_id]

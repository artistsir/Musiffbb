...
async def vector_transport_resolver(url: str) -> str:
    """Fast audio download with streaming support"""
    if os.path.exists(url) and os.path.isfile(url):
        return url

    # Enhanced timeout and retry configuration
    timeout_config = aiohttp.ClientTimeout(total=60, connect=30, sock_connect=30, sock_read=60)
    
    for attempt in range(2):  # Only 2 retries for speed
        try:
                ...
                async with session.get(download_url) as response:
                    if response.status == 200:
                        ...
                        async with aiofiles.open(file_name, 'wb') as f:
                            async for chunk in response.content.iter_chunked(65536):  # Larger chunks for speed
                                await f.write(chunk)
                                file_size += len(chunk)
            ...
            if attempt < 1:
                ...
                # Return URL for direct streaming if download fails
                return url
...
async def fetch_youtube_link(query: str):
    """Fast YouTube search with timeout optimization"""
    # Enhanced timeout for search
    timeout_config = aiohttp.ClientTimeout(total=15, connect=5, sock_connect=5, sock_read=10)
...
async def fetch_youtube_link_backup(query: str):
    """Backup search with fast timeout"""
    ...
    timeout_config = aiohttp.ClientTimeout(total=20, connect=8, sock_connect=8, sock_read=15)
...
current_playback_position = {}  # For seek functionality
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            try:
                ...
                pass
            error_text = f"Error in handler `{func.__name__}` (chat id: {chat_id}):\n\n{str(e)}"
            print(error_text)
            try:
...
async def invite_assistant(chat_id, invite_link, processing_message):
    ...
    try:
        ...
        error_message = f"❌ Error while inviting assistant: {e.code} {e.error_message}"
...
async def help_music_callback(_, callback_query):
    text = (
        ...
        ">➜ `/seek <seconds>`\n"
        "   • Seek forward in the current song. (Admins only)\n\n"
        ">➜ `/seekback <seconds>`\n"
        "   • Seek backward in the current song. (Admins only)"
    )
...
async def help_util_callback(_, callback_query):
    text = (
        ...
        ">➜ `/seek <seconds>`\n"
        "   • Seek forward in current song. (Admins only)\n\n"
        ">➜ `/seekback <seconds>`\n"
        "   • Seek backward in current song. (Admins only)\n\n"
        ">➜ Auto-Suggestions:\n"
...
async def play_handler(_, message: Message):
    ...
    # If replying to an audio/video message
...
async def process_play_command(message: Message, query: str):
    ...
    if isinstance(result, dict) and "playlist" in result:
        ...
        # If it's the first song, start playback immediately
...
async def fallback_local_playback(chat_id: int, message: Message, song_info: dict):
    """Fast playback with direct URL streaming"""
    ...
    try:
        ...
        # Start playback immediately with direct URL streaming
        try:
            await message.edit(f"🎵 Starting: {song_info['title']}...")
            
            # Use direct URL streaming for instant playback
            await call_py.play(
                chat_id,
            ...
            # Set playback position tracking
    ...
    if data == "pause":
        ...
        if chat_id in chat_containers and chat_containers[chat_id]:
            ...
            if chat_id in chat_containers and chat_containers[chat_id]:
                ...
                next_song_info = chat_containers[chat_id][0]
                try:
                    dummy_msg = await bot.send_message(chat_id, f"🎧 Next: **{next_song_info['title']}** ...")
...
async def leave_voice_chat(chat_id):
    ...
    if chat_id in current_playback_position:
        del current_playback_position[chat_id]
...
async def seek_forward_handler(client, message):
    ...
    try:
        ...
        if seek_seconds <= 0:
            await message.reply("❌ Please provide a positive number of seconds.")
            return
        ...
        if new_position >= total_duration:
            await message.reply("❌ Cannot seek beyond the end of the song.")
            return

        # Restart playback from new position
        try:
            await call_py.leave_call(chat_id)
            await asyncio.sleep(1)  # Fast restart
            ...
            await message.reply(f"⏩ Seeked forward by {seek_seconds} seconds.")
        ...
        await message.reply(f"❌ Error during seek: {str(e)}")
...
async def seek_backward_handler(client, message):
    ...
    try:
        ...
        try:
            ...
            await message.reply(f"⏪ Seeked backward by {seek_seconds} seconds.")
...
async def reboot_handler(_, message):
    ...
    try:
        ...
        current_playback_position.pop(chat_id, None)
...
if __name__ == "__main__":
    # Setup logging - COMPLETE FIXED VERSION
    logging.basicConfig(
        level=logging.INFO,

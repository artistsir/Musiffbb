...
current_playback_position = {}  # Track current playback position for seek functionality
...
async def fallback_local_playback(chat_id: int, message: Message, song_info: dict):
    ...
    try:
        ...
        try:
            # Start playback immediately with URL streaming (no download wait)
            await call_py.play(
                chat_id,
...
async def seek_forward_handler(client, message):
    ...
    try:
        ...
        # Check if new position is within song duration
        total_duration = parse_duration_str(song_info.get("duration", "0:00"))
...
async def reboot_handler(_, message):
    ...
    try:
        ...
        # Remove playback position tracking
